...
dispatch_list(struct timer_node *current) {
	do {
		...
		skynet_service_pushmsg(event->handle, &message);
...
skynet_register_timer(uint32_t handle, const void * args, size_t size, int time) {
	if (time <= 0) {
		...
		skynet_service_pushmsg(handle, &message);
	} else {
...
skynet_updatetime(void) {
	...
	if(cp < TI->current_point) {
		skynet_logger_error(NULL, "time diff error: change from %lld to %lld", cp, TI->current_point);
		TI->current_point = cp;
