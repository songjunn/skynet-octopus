...
struct skynet_service * skynet_service_create(const char * name, const char * param, int concurrent) {
...
struct skynet_service * skynet_service_insert(struct skynet_service * ctx, const char * param, int concurrent) {
	SPIN_LOCK(M)
	int index = M->count;
