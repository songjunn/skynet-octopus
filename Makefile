...
DEFINES := -DNOMEMORY_CHECK -D__STDC_NO_ATOMICS__
EXPORT := -Wl,-E
SHARED := -shared -fPIC
...
$(BIN_PATH)/libskynet.so : $(foreach v, $(SKYNET_SRC), skynet-src/$(v))
	$(CC) $(CFLAGS) $(DEFINES) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $(LIBS) $^ -o $@

$(BIN_PATH)/skynet : $(foreach v, $(SKYNET_EXE_SRC), skynet-src/$(v))
	$(CC) $(CFLAGS) $(DEFINES) $(EXPORT) $(JEMALLOC_INC) $(JEMALLOC_LIB) $(LIBS) $^ -o $@ -lskynet -L$(BIN_PATH)

$(LIB_PATH)/liblogger.so : service-src/service_logger.c
	$(CC) $(CFLAGS) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $^ -o $@
...
$(LIB_PATH)/libhttp.so : service-src/service_http.c service-src/http/http_parser.c service-src/http/http_proxy.c
	$(CC) $(CFLAGS) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $^ -o $@ -Iservice-src/http
...
$(LIB_PATH)/libgatews.so : service-src/service_gatews.c service-src/websocket/websocket.c
	$(CC) $(CFLAGS) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $^ -o $@ -Iservice-src/websocket

$(LIB_PATH)/libmongo.so : service-src/service_mongo.c
	$(CC) $(CFLAGS) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $^ -o $@ -Iservice-src/3rd/mongoc/include/libbson-1.0 -Iservice-src/3rd/mongoc/include/libmongoc-1.0 -Lservice-src/3rd/mongoc/lib -lbson-1.0 -lmongoc-1.0

$(LIB_PATH)/libpython.so : service-src/service_python.c
	#$(CC) $(CFLAGS) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $^ -o $@ -Iservice-src/3rd/pypy3/include -Lservice-src/3rd/pypy3/bin -lpypy3-c
	$(CC) $(CFLAGS) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $^ -o $@ -I/usr/include/python3.6m -lpython3.6m

$(LIB_PATH)/libsnlua.so : service-src/service_snlua.c
	$(CC) $(CFLAGS) $(SHARED) $(JEMALLOC_INC) $(JEMALLOC_LIB) $^ -o $@ -Iservice-src/3rd/lua/include -Lservice-src/3rd/lua/lib -llua
